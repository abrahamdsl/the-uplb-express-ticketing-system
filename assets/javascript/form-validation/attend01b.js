/**
*	Javascript validator for Checking Out
* 	Created 17MAR2012-1339
		...
		success: function(data){
			...
			{								
				...
				$( splitted[1] ).find( 'guest' ).each( function() {			
					var alreadyEntered = ( $(this).find( 'exited' ).text() == "1" );
		...
		$("#buttonOK2").click( function() {			
			...
			if( $('input[type="checkbox"]:checked').size() < 1 )
			{
				$.fn.nextGenModal({
				   ...
				   message: 'Select at least one guest to confirm exit',
				});	
				return false;
			}
			if( isThereGuest !== 1 ){
			...
			$.fn.nextGenModal({
			   msgType: 'warning',
			   title: 'confirm entry?',
			   message: 'Confirm exit of the checked guests? ',
