...
(function($){
    ...
    $.fn.drag_drop_multi_select = function(settings){
        ...
        return this.each(function(i) {
            ...
            $self.find(config.element_to_drag_drop_select).each(function(){
				...
				*/
				$item.unbind();
				if( config.lassoAvailable === false ){ 
					if( $(this).hasClass( config.occupiedSameClassIndicator ) )
					{	// since seat is occupied already, no need to continue further down the road					
						return 1;	// returning non-false value is the same as 'continue' in a regular loop
					}
				}
                ...
                $item.bind('ddms.select',function(){					
					...
					if( $(this).hasClass( config.otherGuestIndicator) )
					{
						$(document).showWarning(config, 'Seat assigned to to other guest in this booking.' , false );							
						return false;
					}
					if( config.lassoAvailable )
					{
						/*
							We are in Create Event Step 5.
						*/
						...
						/*
							We are selecting seat for booking
							This part was created on 13FEB2012. DOM dependent, on Book Step 4.
						*/
						manipulateGuestSeat( "SELECT", $(this).attr('id') );	// found in bookStep4.js		
					}
					...
					if( config.lassoAvailable ) $(config.element_to_show_result).text($self.find('.ui-selected').size());					
                });
            ...
            // MAKE ITEMS SELECTABLE
				...
				$self.selectable({				
					...
					start: function( ev, ui ){										
						...
						if( config.lassoAvailable ) return false;				
    ...
    $.fn.drag_drop_multi_select.defaults={
		...
		occupiedSameClassIndicator: 'occupiedSameClass', // aded by abe 13FEB2012-2350 - if other user of the same class has booked the seat already
		otherGuestIndicator: 'otherGuest', //added by abe 13FEB2012 - in booking-pick a seat used to check if certain seat is already picked in the same booking and thus cannot be	picked anymore.
		...
		lassoAvailable: true, // added by abe 13FEB2012 - this is when client chooses seat in booking a ticket. Should be specified to false if he/she is booking.
