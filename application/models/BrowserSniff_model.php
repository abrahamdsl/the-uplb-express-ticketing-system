...
/**
 *  Browser Sniffing Model
 *  Created 25MAY2012-0023
 *  Third-party plugin used and modified, under GNU GPL
 ...
 *	------------------------------
 *
 *	Various functions relating to browser sniffing for the system.
 *  Specifically, we determine the visitor's browser here and determine approriate reaction
 *  to be returned to calling Controller.
**/
...
class BrowserSniff_model extends CI_Model {	
	...
	function browser_detection( $which_test, $test_excludes='', $external_ua_string='' ) 
	{
		...
		*/
		if ( !$b_repeat ) {
			...
			*/
			$a_webkit_types = array( 'arora', 'chromium', 'chrome', 'epiphany', 'gtklauncher', 'icab', 'konqueror', 'maxthon',  'midori', 'omniweb', 'qupzilla', 'rekonq', 'safari', 'shiira', 'uzbl', 'applewebkit', 'webkit' );
 ...
 function decideActionOnUserAgent()
 {
	/**
	...
	*/
	...
	$sniffedData = $this->browser_detection( 'full', '', '' ) ;
	// normal or mobile browser (i.e., iPhone )
	if( $sniffedData[8] == 'bro' or $sniffedData[8] == 'mobile' )
	{
		switch( $sniffedData[0] )
		{			
			case 'ie':
					/* 	IE'S realm 
						IE 8 and earlier are NOT allowed.
						Meaning, Windows XP users cannot access the system using IE (since IE8 is the last IE to run on XP).
						HAHAHAHA!!!
					*/
					...
					*/
					...
					switch( $sniffedData[11][0] )
					{
						case 'chrome':
							...
							break;
						case 'safari': 
							if( $ver_split[0] < 533 and $ver_split[1] <= 21 ) 
								return BR_NOT_TESTED_BUT_PERMIT_STILL; 
							else 
								return BR_ALLOWED;  
							break;						
						default:
							return BR_UNKNOWN_BUT_PERMIT_STILL;
					}
						break;
			case 'moz':
					/*
						Mozilla browsers: Firefox and SeaMonkey
						
						Firefox is not allowed if version is less than 3.6
						SeaMonkey would show not tested warning but still permit if version is less than 2.9
					*/
					$ver_split =  explode( '.', $sniffedData[10][1] );
					switch( $sniffedData[10][0] )
					{
						case 'firefox': 
							...
							break;
						case 'seamonkey':
							if( $ver_split[0] <= 2 and $ver_split[1] < 9 ) 
								return BR_NOT_TESTED_BUT_PERMIT_STILL; 
							else 
								return BR_ALLOWED;  
							break;
			...
			case 'op':
					/*
						...
						For major versions less than 11, show not tested warning but still permit.
					*/
					$ver_split =  explode( '.', $sniffedData[1] );
					if( $ver_split[0] < 11 )					
						return BR_NOT_TESTED_BUT_PERMIT_STILL;
						...
						return BR_ALLOWED;					
					break;
			default: return BR_UNKNOWN_BUT_PERMIT_STILL;
		}
	}else
	if(  $sniffedData[8] == 'bot')
	{		
		return BR_BOT_SIMPLE;
	}else{		
		return BR_AGENT_DENIED;
	}	
 }//sniffUserAgent(..)
